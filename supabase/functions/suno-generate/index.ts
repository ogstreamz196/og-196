...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Base price plus 1 coin for each minute beyond the included 3.
    const extraMinutes = Math.max(0, targetMinutes - 3);
    let coinCost = (await getSetting(admin, "coins_per_generation", 3)) + extraMinutes;
